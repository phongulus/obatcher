module IntRbtree = Data.Rbtree.Make(Int)
...
let generic_run test_spec f =
  let old_search_threshold = !Data.Rbtree.rbtree_search_sequential_threshold in
  let old_insert_threshold = !Data.Rbtree.rbtree_search_sequential_threshold in
  (match test_spec.args.search_threshold with None -> () | Some st ->
    Data.Rbtree.rbtree_search_sequential_threshold := st);
  (match test_spec.args.insert_threshold with None -> () | Some it ->
    Data.Rbtree.rbtree_insert_sequential_threshold := it);
  let res = f () in
  Data.Rbtree.rbtree_search_sequential_threshold := old_search_threshold;
  Data.Rbtree.rbtree_insert_sequential_threshold := old_insert_threshold;
  res
...
module Sequential = struct
  ...
  let init _pool test_spec =
    generic_init test_spec (fun initial_elements ->
      ...
      Array.iter (fun i -> IntRbtree.Sequential.insert (IntRbtree.Sequential.new_node i ()) tree)
        initial_elements;
      tree
    )
  ...
  let run _pool t test_spec =
    ...
    Array.iter (fun i ->
        IntRbtree.Sequential.insert (IntRbtree.Sequential.new_node i ()) t
