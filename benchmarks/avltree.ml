module IntAvltree = Data.Avltree.Make(Int)
...
let generic_run test_spec f =
  let old_search_threshold = !Data.Avltree.avltree_search_sequential_threshold in
  let old_insert_threshold = !Data.Avltree.avltree_search_sequential_threshold in
  (match test_spec.args.search_threshold with None -> () | Some st ->
    Data.Avltree.avltree_search_sequential_threshold := st);
  (match test_spec.args.insert_threshold with None -> () | Some it ->
    Data.Avltree.avltree_insert_sequential_threshold := it);
  let res = f () in
  Data.Avltree.avltree_search_sequential_threshold := old_search_threshold;
  Data.Avltree.avltree_insert_sequential_threshold := old_insert_threshold;
  res
