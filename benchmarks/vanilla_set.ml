    ...
    Domainslib.Task.parallel_for pool ~chunk_size:1
      ...
      ~body:(fun i ->
        ...
        Fun.protect ~finally:(fun () -> Mutex.unlock t.lock) (fun () ->
          ...
          then t.set <- IntSet.add test_spec.insert_elements.(i) t.set
          else ignore (IntSet.mem test_spec.search_elements.(i - Array.length test_spec.search_elements) t.set)
        )
      )
...
module Batched = struct
  ...
  module IntSetWrapper = struct
    ...
    let run (t: t) (pool: Domainslib.Task.pool) (ops: wrapped_op array) =
      ...
      Domainslib.Task.parallel_for pool ~start:0 ~finish:(Array.length searches - 1) ~body:(fun i ->
        ...
        kont (IntSet.mem key set)
      );
      let inserts = Array.of_list !inserts in
      for i = 0 to Array.length inserts - 1 do
        t := IntSet.add inserts.(i) !t
      done

  end

  module BatchedIntSet = Domainslib.Batcher.Make (IntSetWrapper)

  type t = BatchedIntSet.t
  ...
  let init pool test_spec =
    generic_init test_spec (fun initial_elements ->
      let set = BatchedIntSet.init pool in
      for i = 0 to Array.length initial_elements - 1 do
        BatchedIntSet.apply set (Insert initial_elements.(i));
      done;
      set
    )
  ...
  let run pool (t: t) test_spec =
    Domainslib.Task.parallel_for pool ~chunk_size:1
      ...
      ~body:(fun i ->
        if i < Array.length test_spec.insert_elements
        then BatchedIntSet.apply t (Insert test_spec.insert_elements.(i))
        else ignore (BatchedIntSet.apply t (Member test_spec.search_elements.(i - Array.length test_spec.search_elements)))
      )
